  ...
  return (
    <main className="dashboard-page">
      ...
      <section className="dashboard-content">
        ...
        {!loading && !error && posts.length > 0 && <div className="dashboard-story-list">{posts.map((post, index) => <article className="dashboard-story" key={post.id}><div className="dashboard-story-number">{String(index + 1).padStart(2, '0')}</div><div className="dashboard-story-main"><span className="dashboard-status">{post.published ? 'Published' : 'Draft'}</span><h2><a href={`/article/${post.id}`}>{post.title}</a></h2><p>{excerpt(post.content)}</p><div className="dashboard-story-meta"><span>{post.published ? 'Published story' : 'Only you can see this'}</span><i /><a href={`/article/${post.id}`}>Read story ↗</a><a href={`/edit/${post.id}`}>Edit</a></div></div></article>)}</div>}
      </section>
